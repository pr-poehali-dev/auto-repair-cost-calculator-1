...
def handler(event: dict, context) -> dict:
    ...
    cur.execute("""
        SELECT id, generation_id, name, engine, transmission, power,
               engine_type, engine_code, drive_type,
               front_brakes, rear_brakes, front_suspension, rear_suspension, turbo_type
        FROM car_modifications ORDER BY name
    """)
    for r in cur.fetchall():
        if r[1] in gens:
            gens[r[1]]["modifications"].append({
                ...
                "frontBrakes": r[9] or "", "rearBrakes": r[10] or "",
